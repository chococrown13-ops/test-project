...
| 자막 | 화면 위쪽 (아래 배치표 참고), 굵은 흰 글씨 + 검은 테두리. 강조 단어는 노란색 (수첩 색과 통일) |
...
쇼츠·릴스는 화면 아래 약 20%를 채널 이름·캡션이, 오른쪽을 좋아요·댓글 버튼이 가립니다.
완성된 이미지들은 모두 위쪽에 하늘이나 빈 물이 있어서 **자막을 위쪽에 두면 한 번도 겹치지 않습니다.**

| 장면 | 이미지 | 자막 위치 | 편집 메모 |
|---|---|---|---|
| S1 | ✅ | 맨 위 10% (물음표 위) | 빠른 줌인 + "띠용" |
| S2 | ✅ | 위쪽 15~25% | 수첩 넘기는 소리 |
| S3 | ✅ | 위쪽 15~25% (하늘) | 옆으로 천천히 흐르는 이동 |
| S4 | ✅ | 위쪽 15~25% (하늘) | 위→아래 틸트: 해달 → 해초 줄기 → 성게 바닥 |
| S5 | ✅ | 맨 위 5~10% | 이미지 90%로 줄여 위로 올리고 아래는 캔버스 '배경 흐림'. 손잡은 두 마리로 줌인 |
| S6 | ✅ | 위쪽 10~20% | "손잡고 잔다 → 해초 이불 덮고 잔다" 자막. 연필 소리 |
| S7 | ✅ | 위쪽 10~20% | 정면 고정. 매 영상 재사용 |
